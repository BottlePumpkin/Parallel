...
## Task 1: Xcode 프로젝트 생성·SwiftTerm 의존성 추가

**Files:**
- Create: `Parallel.xcodeproj` (Xcode 템플릿)
- Create: `Parallel/ParallelApp.swift`
- Create: `Parallel/Views/ContentView.swift`

- [ ] **Step 1: Xcode에서 macOS App 프로젝트 생성**

Xcode → File → New → Project → macOS → App
- Product Name: `Parallel`
- Bundle Identifier: `com.byeonghopark.parallel`
- Interface: SwiftUI
- Language: Swift
- Storage: None
...
- [ ] **Step 2: App Sandbox 끄기**

Project Settings → Parallel target → Signing & Capabilities → "App Sandbox" capability 제거.
(spec §9 #9: 임의 디렉토리 접근 필요)

- [ ] **Step 3: SwiftTerm SPM 추가**

File → Add Package Dependencies → `https://github.com/migueldeicaza/SwiftTerm` → Up to Next Major → Add to Parallel target.
...
Run: ⌘R
Expected: "Parallel" 텍스트가 윈도우에 보임.

- [ ] **Step 5: Commit**

```bash
git add .
git commit -m "chore: scaffold Parallel macOS app + SwiftTerm dependency"
```
